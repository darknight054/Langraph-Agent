    ...
    request_item = {
        ...
        "multi_modal_data": {
            "image": DeepseekOCRProcessor().tokenize_with_images(
                prompt=prompt, images=[image], bos=True, eos=True, cropping=CROP_MODE
            )
        },
    }
...
async def process_image_endpoint(
    ...
    try:
        ...
        return OCRResponse(success=True, result=result, page_count=1)
    ...
    try:
        ...
        if not images:
            ...
            return BatchOCRResponse(
                success=False, results=[], total_pages=0, filename=file.filename
            )
        ...
        for page_num, image in enumerate(tqdm(images, desc="Processing pages")):
            try:
                ...
                result = process_single_image(image, use_prompt)
                results.append(
                    OCRResponse(success=True, result=result, page_count=page_num + 1)
                )
                print(
                    f"[DEBUG] Page {page_num + 1} processed successfully, output length: {len(result)}"
                )
            except Exception as e:
...
async def process_batch_endpoint(
    files: List[UploadFile] = File(...), prompt: Optional[str] = Form(None)
):
